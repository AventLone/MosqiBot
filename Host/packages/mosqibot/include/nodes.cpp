...
RosClient::RosClient(std::string name) : Node(name), taskflag(false)
{
    ...
    target_img_publisher = this->create_publisher<sensor_msgs::msg::Image>("MosqiBot/target_img", 10);
    heap_img_publisher = this->create_publisher<sensor_msgs::msg::Image>("MosqiBot/heap_img", 10);

    std::string this_package_path = ament_index_cpp::get_package_share_directory("mosqibot");
    projector = std::make_unique<Projector>(this_package_path + "/param/camera_info.yaml", tvec, depth);
...
void RosClient::callback_subsription(const sensor_msgs::msg::Image::ConstSharedPtr msg)
{
    ...
    projector->detect(cv_ptr->image, target_img);
    projector->locateHeap(cv_ptr->image, heap_img);

    target_img_msg = *cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", target_img).toImageMsg();
    heap_img_msg = *cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", heap_img).toImageMsg();

    target_img_publisher->publish(target_img_msg);
    heap_img_publisher->publish(heap_img_msg);
}
}   // namespace MosqiBot
